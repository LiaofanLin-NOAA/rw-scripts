...
cd /scratch3/BMC/rtwrfruc/RRFSv2X/NCO_dirs/RRFSv2X/exp/rrfsenkf

# Email recipient and subject
recipients="Liaofan.Lin@noaa.gov Liaofan.Lin@noaa.gov"
subject="rrfsv2x alert - enkf "
...
echo "${msg}" > ${script_dir}/ensmsg.new

# need to send out alerts when we get DEAD jobs
if [[ -s ${script_dir}/ensmsg.save  && -s ${script_dir}/ensmsg.new ]]; then
  # only send alerts if the new msg is different from the saved one to avoid duplicate alerts
  if ! diff ${script_dir}/ensmsg.save ${script_dir}/ensmsg.new &>/dev/null && [[ ${msg} == *DEAD* ]]; then
    send_email=true
  fi
elif [[ ${msg} == *DEAD* ]]; then
  send_email=true
fi
mv ${script_dir}/ensmsg.new ${script_dir}/ensmsg.save

## check whether if the workflow is stalled, i.e. no running jobs
## and if stalled for more than an hour, send out alerts
if [[ ${msg} != *RUNNING* ]]; then
  if [[ -s ${script_dir}/ensstall ]]; then
    stall_bgn=$(head -n 1 ${script_dir}/ensstall)
    cur_seconds=$(date +%s)
    diff_secons=$(( cur_seconds - stall_bgn ))
    if (( diff_secons > 3600 )); then # stalled for an hour
      send_email=true
      rm -rf ${script_dir}/ensstall
    fi
  else
    date +%s > ${script_dir}/ensstall
  fi
fi
